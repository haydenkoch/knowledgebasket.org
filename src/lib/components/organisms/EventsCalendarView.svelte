		...
		>
			<div class="kb-event-mobile-drawer-content">
				...
				{#if calendarSelectedEvent.startDate}
					<p class="kb-drawer-date">{new Date(calendarSelectedEvent.startDate).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
				{/if}
...
<style>
.kb-calendar-view { width: 100%; min-height: 400px; }
.kb-calendar-loading {
	display: flex;
