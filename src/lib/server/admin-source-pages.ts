...
function parseTagsFromTextarea(value: FormDataEntryValue | null) {
	const raw = (value as string | null)?.trim();
	if (!raw) return null;
	return raw
		.split(/\r?\n/)
		...
		.filter((tag) => tag.tagKey.length > 0);
}

export async function loadSourceOpsDetail(sourceId: string) {
	const detail = await getSourceById(sourceId);
	if (!detail) return null;

	const [fetchLogs, batches, candidates, sourceRuns] = await Promise.all([
		getFetchLogsForSource(sourceId, 10),
		db
			.select()
			.from(importBatches)
			.where(eq(importBatches.sourceId, sourceId))
			.orderBy(desc(importBatches.startedAt))
			.limit(10),
		getRecentCandidatesForSource(sourceId, 10),
		getRecentSourceRuns(sourceId, 8)
	]);
...
export function _createSourceConfigActions(
	deps: SourceDetailActionDeps = { updateSource, previewSource, ingestSource, runSourceNow }
) {
	return {
		updateSource: async ({ params, request }: { params: { id: string }; request: Request }) => {
			...
			try {
				await deps.updateSource(params.id, {
					...
					tags:
						parseTagsFromTextarea(fd.get('tagsText')) ??
						(fd.getAll('tagKey') as string[]).map((tagKey, index) => ({
							tagKey,
		...
		pauseSource: async ({ params, request }: { params: { id: string }; request: Request }) => {
			const fd = await request.formData();
			await deps.updateSource(params.id, {
				status: 'paused',
				pausedAt: new Date(),
		...
		resumeSource: async ({ params }: { params: { id: string } }) => {
			await deps.updateSource(params.id, {
				status: 'active',
				pausedAt: null,
				pauseReason: null
		...
		quarantineSource: async ({ params, request }: { params: { id: string }; request: Request }) => {
			const fd = await request.formData();
			await deps.updateSource(params.id, {
				quarantined: true,
				quarantineReason:
		...
		unquarantineSource: async ({ params }: { params: { id: string } }) => {
			await deps.updateSource(params.id, {
				quarantined: false,
		...
		disableSource: async ({ params }: { params: { id: string } }) => {
			await deps.updateSource(params.id, {
				status: 'disabled',
				enabled: false
			});
		...
		deprecateSource: async ({ params }: { params: { id: string } }) => {
			await deps.updateSource(params.id, {
				status: 'deprecated',
...
export function _createSourceRunActions(
	deps: SourceDetailActionDeps = { updateSource, previewSource, ingestSource, runSourceNow }
) {
	return {
		testSource: async ({ params }: { params: { id: string } }) => {
			try {
		...
		runImport: async ({ params }: { params: { id: string } }) => {
			try {
		...
		retrySource: async ({ params, locals }: { params: { id: string }; locals: App.Locals }) => {
			try {
...
export function _createSourceDetailActions(
	...
	return {
		..._createSourceConfigActions(deps),
		..._createSourceRunActions(deps)
	};
}
