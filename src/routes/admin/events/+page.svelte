...
<div class="space-y-6">
	<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
		...
		<div class="flex flex-wrap items-center gap-2">
			<a href="/admin/events/export?format=csv&status={data.currentStatus}&search={encodeURIComponent(data.currentSearch ?? '')}&sort={data.currentSort}&order={data.currentOrder}">
				<Button type="button" variant="outline">Export CSV</Button>
			</a>
			<a href="/admin/events/export?format=ical&status={data.currentStatus}&search={encodeURIComponent(data.currentSearch ?? '')}&sort={data.currentSort}&order={data.currentOrder}">
				<Button type="button" variant="outline">Export iCal</Button>
			</a>
			<span class="text-muted-foreground text-sm">Exports current filter ({data.total} events)</span>
	...
	<div class="flex flex-wrap items-center gap-4">
		<form onsubmit={(e) => { e.preventDefault(); doSearch(); }} class="flex gap-2">
			<Input type="text" bind:value={searchValue} placeholder="Search events..." class="max-w-sm" />
			<Button type="submit" variant="secondary">Search</Button>
		</form>
		<div class="flex items-center gap-2">
			<label for="sort-by" class="text-muted-foreground text-sm">Sort by</label>
			<NativeSelect.Root
				id="sort-by"
	...
	{#if selectedIds.length > 0}
		<Alert>
			<AlertDescription class="flex flex-wrap items-center gap-2">
				...
				<form method="POST" action="?/bulkApprove" use:enhance={() => ({ result, update }) => {
					...
					{/each}
					<Button type="submit" size="sm" variant="default" class="bg-green-600 hover:bg-green-700">Approve</Button>
				</form>
				...
				<AlertDialog.Root>
					...
					<AlertDialog.Content>
						...
						<AlertDialog.Footer>
							...
							<form method="POST" action="?/bulkDelete" use:enhance={() => ({ result, update }) => {
			...
			<Table.Body>
				{#each data.events as event}
					<Table.Row>
						...
						<Table.Cell class="text-muted-foreground">{event.organizationName ?? event.hostOrg ?? '—'}</Table.Cell>
						<Table.Cell class="text-muted-foreground">{event.startDate ?? '—'}</Table.Cell>
						<Table.Cell class="max-w-[120px] truncate text-muted-foreground text-xs">
							{#if event.createdAt}
								{new Date(event.createdAt).toLocaleDateString()}
								{#if event.submitterName || event.submitterEmail}
									<br /><span class="text-muted-foreground">{event.submitterName ?? event.submitterEmail ?? 'Public'}</span>
								{/if}
							{:else}
								—
							{/if}
						...
						<Table.Cell class="text-right">
							<div class="flex items-center justify-end gap-1">
								...
								{#if event.slug}
									<Button href="/events/{event.slug}" variant="ghost" size="icon-sm" title="View" target="_blank">
										<Eye class="h-4 w-4" />
									</Button>
								{/if}
								{#if event.contactEmail}
									<Button href="mailto:{event.contactEmail}" variant="ghost" size="icon-sm" title="Contact">
										<Mail class="h-4 w-4" />
									</Button>
								{/if}
							</div>
