<script lang="ts">
	...
	const statuses = ['all', 'draft', 'pending', 'published', 'rejected', 'cancelled'];
	...
	function setAllSelected(checked: boolean) {
		selectedIds = checked ? data.funding.map((item) => item.id) : [];
	}
	...
	const allSelected = $derived(
		data.funding.length > 0 && selectedIds.length === data.funding.length
	);
</script>
...
<div class="space-y-6">
	...
	>
		{#snippet actions()}
			<Button href="/admin/funding/new">
				<Plus class="mr-2 h-4 w-4" />
				Add funding
			</Button>
		{/snippet}
		{#snippet meta()}
			<span>{data.total} funding item{data.total !== 1 ? 's' : ''} match current filter</span>
		{/snippet}
	</AdminPageHeader>
	...
	{#if data.total === 0 && !data.currentSearch}
		<Empty.Root>
			<Empty.Header>
				<Empty.Title>No funding listings yet</Empty.Title>
				<Empty.Description>Create your first funding listing to get started.</Empty.Description>
			</Empty.Header>
			<Empty.Content>
				<Button href="/admin/funding/new">
					<Plus class="mr-2 h-4 w-4" />
	...
	{:else}
		<AdminSectionCard title="Funding">
			{#snippet children()}
					...
					<div class="flex flex-wrap items-center gap-3">
						...
						<div class="flex items-center gap-2 text-sm text-[var(--mid)]">
							...
							>
								...
								<NativeSelect.Option value="deadline">Deadline</NativeSelect.Option>
							...
							<div class="ml-auto flex flex-wrap items-center gap-2">
								...
								<AlertDialog.Root>
									...
									<AlertDialog.Content>
										<AlertDialog.Header>
											<AlertDialog.Title>
												Delete {selectedIds.length} funding item{selectedIds.length === 1
													? ''
													: 's'}?
											</AlertDialog.Title>
											<AlertDialog.Description>
												This cannot be undone. The selected funding listings will be permanently
												deleted.
											</AlertDialog.Description>
										</AlertDialog.Header>
										<AlertDialog.Footer>
								...
								<Table.Row>
									<Table.Head class="w-10">
										<Checkbox
											...
											aria-label="Select all funding items"
										/>
									</Table.Head>
