<script lang="ts">
	...
	function setAllSelected(checked: boolean) {
		selectedIds = checked ? data.jobs.map((item) => item.id) : [];
	}
	...
	const allSelected = $derived(data.jobs.length > 0 && selectedIds.length === data.jobs.length);
</script>
...
<div class="space-y-6">
	...
	>
		...
		{/snippet}
		{#snippet meta()}
			<span>{data.total} job{data.total !== 1 ? 's' : ''} match current filter</span>
		{/snippet}
	</AdminPageHeader>
	...
	{#if data.total === 0 && !data.currentSearch}
		<Empty.Root>
			<Empty.Header>
				...
				<Empty.Description>Create your first job listing to get started.</Empty.Description>
			</Empty.Header>
			<Empty.Content>
				<Button href="/admin/jobs/new">
					<Plus class="mr-2 h-4 w-4" />
						...
						>
							...
							<div class="ml-auto flex flex-wrap items-center gap-2">
								<form
									...
									use:enhance={() =>
										({ result, update }) => {
											if (result.type === 'success') toast.success('Jobs approved');
											update();
										}}
									class="contents"
								>
								...
								<form
									...
									use:enhance={() =>
										({ result, update }) => {
											if (result.type === 'success') toast.success('Jobs rejected');
											update();
										}}
									class="contents"
								>
								...
								<AlertDialog.Root>
									...
									<AlertDialog.Content>
										<AlertDialog.Header>
											<AlertDialog.Title>
												Delete {selectedIds.length} job{selectedIds.length === 1 ? '' : 's'}?
											</AlertDialog.Title>
											<AlertDialog.Description>
												This cannot be undone. The selected job listings will be permanently
												deleted.
											</AlertDialog.Description>
										</AlertDialog.Header>
										<AlertDialog.Footer>
											...
											<form
												...
												use:enhance={() =>
													({ result, update }) => {
														if (result.type === 'success') toast.success('Jobs deleted');
														update();
													}}
												class="contents"
											>
