		...
		<div class="flex flex-wrap gap-2">
			{#if data.business.status === 'pending'}
				<form
					...
					use:enhance={() =>
						({ result, update }) => {
							if (result.type === 'success') toast.success('Listing approved');
							update();
						}}
				>
				...
				<AlertDialog.Root>
					...
					<AlertDialog.Content>
						<AlertDialog.Header>
							<AlertDialog.Title>Reject listing</AlertDialog.Title>
						...
						<form
							...
							use:enhance={() =>
								({ result, update }) => {
									if (result.type === 'success') toast.success('Listing rejected');
									update();
								}}
							class="space-y-3"
						>
							...
							<textarea
								...
								placeholder="e.g. Eligibility is unclear or listing needs revision"
							></textarea>
							<AlertDialog.Footer>
				...
				<AlertDialog.Content>
					...
					<AlertDialog.Footer>
						...
						<form
							...
							use:enhance={() =>
								({ result, update }) => {
									if (result.type === 'success') toast.success('Listing deleted');
									update();
								}}
						>
							<AlertDialog.Action
