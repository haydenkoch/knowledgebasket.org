<script lang="ts">
	...
	function setAllSelected(checked: boolean) {
		selectedIds = checked ? data.resources.map((item) => item.id) : [];
	}
	...
	const allSelected = $derived(
		data.resources.length > 0 && selectedIds.length === data.resources.length
	);
</script>
...
<div class="space-y-6">
	...
	>
		{#snippet actions()}
			<Button href="/admin/toolbox/new">
				<Plus class="mr-2 h-4 w-4" />
				Add resource
			</Button>
		{/snippet}
		{#snippet meta()}
			<span>{data.total} resource{data.total !== 1 ? 's' : ''} match current filter</span>
		{/snippet}
	</AdminPageHeader>
	...
	{#if data.total === 0 && !data.currentSearch}
		<Empty.Root>
			<Empty.Header>
				<Empty.Title>No toolbox resources yet</Empty.Title>
				<Empty.Description>Create your first resource to get started.</Empty.Description>
			</Empty.Header>
			<Empty.Content>
				<Button href="/admin/toolbox/new">
					<Plus class="mr-2 h-4 w-4" />
	...
	{:else}
		<AdminSectionCard title="Toolbox">
			{#snippet children()}
						...
						>
							...
							<div class="ml-auto flex flex-wrap items-center gap-2">
								<form
									...
									use:enhance={() =>
										({ result, update }) => {
											if (result.type === 'success') toast.success('Resources approved');
											update();
										}}
									class="contents"
								>
								...
								<form
									...
									use:enhance={() =>
										({ result, update }) => {
											if (result.type === 'success') toast.success('Resources rejected');
											update();
										}}
									class="contents"
								>
								...
								<AlertDialog.Root>
									...
									<AlertDialog.Content>
										<AlertDialog.Header>
											<AlertDialog.Title>
												Delete {selectedIds.length} resource{selectedIds.length === 1 ? '' : 's'}?
											</AlertDialog.Title>
											<AlertDialog.Description>
												This cannot be undone. The selected toolbox resources will be permanently
												deleted.
											</AlertDialog.Description>
										</AlertDialog.Header>
										<AlertDialog.Footer>
											...
											<form
												...
												use:enhance={() =>
													({ result, update }) => {
														if (result.type === 'success') toast.success('Resources deleted');
														update();
													}}
												class="contents"
											>
