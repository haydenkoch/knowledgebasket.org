<script lang="ts">
	...
	const canonicalUrl = $derived(item?.slug ? `${origin}/red-pages/${item.slug}` : `${origin}/red-pages`);
...
<svelte:head>
	<title>{item ? `${item.title} | Red Pages | Knowledge Basket` : 'Listing not found | Knowledge Basket'}</title>
	{#if item}
		...
		<aside class="flex flex-col gap-4">
				...
				<div class="flex flex-col gap-2">
					{#if item.website}
						<Button href={item.website} target="_blank" rel="noopener" class="w-full">
							<GlobeIcon class="size-4 mr-1.5" /> Visit website
						</Button>
					{/if}
					{#if item.phone}
						<Button variant="outline" href="tel:{item.phone}" class="w-full">
							<PhoneIcon class="size-4 mr-1.5" /> {item.phone}
						</Button>
					{/if}
					{#if item.email}
						<Button variant="outline" href="mailto:{item.email}" class="w-full">
							<MailIcon class="size-4 mr-1.5" /> Email
						</Button>
					{/if}
				</div>
			{/if}
			...
			<div class="rounded-xl border border-[var(--border)] bg-[var(--card)] p-5">
				...
				<dl class="flex flex-col gap-3 text-sm">
					{#if item.tribalAffiliation}
						<div>
							<dt class="font-semibold text-[var(--muted-foreground)] text-[11px] uppercase tracking-[0.06em]">Tribal affiliation</dt>
							<dd class="text-[var(--foreground)] mt-0.5">{item.tribalAffiliation}</dd>
						</div>
					{/if}
					{#if serviceTypesDisplay.length}
						<div>
							<dt class="font-semibold text-[var(--muted-foreground)] text-[11px] uppercase tracking-[0.06em]">Service type</dt>
							<dd class="text-[var(--foreground)] mt-0.5">{serviceTypesDisplay.join(', ')}</dd>
						</div>
					{/if}
					{#if item.region}
						<div>
							<dt class="font-semibold text-[var(--muted-foreground)] text-[11px] uppercase tracking-[0.06em]">Service area</dt>
							<dd class="text-[var(--foreground)] mt-0.5">{item.region}</dd>
						</div>
					{/if}
					{#if addressLine}
						<div>
							<dt class="font-semibold text-[var(--muted-foreground)] text-[11px] uppercase tracking-[0.06em]">Address</dt>
							<dd class="text-[var(--foreground)] mt-0.5">
								<span class="flex items-start gap-1.5">
									<MapPinIcon class="size-3.5 mt-0.5 shrink-0 text-[var(--muted-foreground)]" />
									<span>
										{addressLine}
										{#if mapsUrl}
											<a href={mapsUrl} target="_blank" rel="noopener" class="block text-xs text-[var(--teal)] mt-0.5">Get directions</a>
										{/if}
									</span>
								</span>
							</dd>
					...
					{#if item.certifications?.length}
						<div>
							<dt class="font-semibold text-[var(--muted-foreground)] text-[11px] uppercase tracking-[0.06em]">Certifications</dt>
							<dd class="mt-1 flex flex-wrap gap-1">
								{#each item.certifications as cert}
									<span class="inline-block px-2 py-0.5 text-[11px] rounded bg-[var(--muted)] text-[var(--muted-foreground)]">{cert}</span>
								{/each}
							</dd>
						</div>
					{/if}
					{#if item.ownerName}
						<div>
							<dt class="font-semibold text-[var(--muted-foreground)] text-[11px] uppercase tracking-[0.06em]">Owner</dt>
							<dd class="text-[var(--foreground)] mt-0.5">{item.ownerName}</dd>
						</div>
					{/if}
				</dl>
			</div>
			...
			{#if item.socialLinks && Object.keys(item.socialLinks).length > 0}
				<div class="rounded-xl border border-[var(--border)] bg-[var(--card)] px-5 py-4">
					<h3 class="font-sans text-xs font-bold uppercase tracking-wider text-[var(--muted-foreground)] mb-2">Social</h3>
