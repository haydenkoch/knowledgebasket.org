...
{/snippet}
{#snippet stats()}
	<div class="font-sans text-white"><strong class="text-[28px] font-bold block leading-none">{total}</strong><span class="text-xs opacity-70">Resources</span></div>
	<div class="font-sans text-white"><strong class="text-[28px] font-bold block leading-none">{mediaTypeValues.length}</strong><span class="text-xs opacity-70">Media types</span></div>
{/snippet}
{#snippet sidebar()}
...
<div>
	...
	<KbTwoColumnLayout {sidebar}>
		{#snippet children()}
		<div class="flex flex-wrap gap-2 mb-4 pb-4 border-b border-[var(--rule)]" role="tablist">
				{#each TOOLBOX_SUBSECTIONS as sub}
					<button
						type="button"
						class="text-sm font-semibold px-3 py-1.5 rounded border cursor-pointer transition-colors {activeSubsection === sub.id ? 'bg-[var(--color-lakebed-950)] text-white border-[var(--color-lakebed-950)]' : 'border-[var(--border)] text-[var(--muted-foreground)] bg-transparent hover:bg-[var(--accent)]'}"
						role="tab"
			...
			<div class="flex items-center justify-between mb-[22px] pb-4 border-b border-[var(--rule)]">
				<div class="font-sans text-[14px] text-[var(--muted-foreground)]">Showing <strong class="text-[var(--dark)]">{filteredTotal}</strong> resources</div>
			...
			<div class="flex flex-col gap-3">
				{#each paginatedList as resource (resource.id)}
					<a href="/toolbox/{resource.slug ?? resource.id}" class="flex gap-4 items-start bg-white border border-[var(--rule)] rounded-lg p-4 no-underline text-inherit transition-shadow duration-150 hover:shadow-[var(--shh)]">
						<div class="text-3xl flex-none w-10 text-center leading-none" aria-hidden="true">{iconFor(resource)}</div>
						<div style="flex: 1; min-width: 0;">
							<span class="font-serif text-base font-semibold text-[var(--dark)] block mb-1">{resource.title}</span>
							{#if resource.source}<div class="text-sm text-[var(--muted-foreground)] mb-1">{resource.source}</div>{/if}
							{#if resource.description}<div class="text-[13px] leading-[1.5] text-[var(--mid)] mb-2 line-clamp-2">{stripHtml(String(resource.description))}</div>{/if}
							<div class="flex flex-wrap gap-1.5">
								{#if resource.mediaType}<span class="text-[11px] font-semibold px-2 py-0.5 rounded bg-[var(--muted)] text-[var(--muted-foreground)]">{resource.mediaType}</span>{/if}
								{#if resource.category}<span class="text-[11px] font-semibold px-2 py-0.5 rounded bg-[var(--muted)] text-[var(--muted-foreground)]">{resource.category}</span>{/if}
							</div>
						</div>
					</a>
				{/each}
	...
	<div class="flex items-center justify-between gap-6 px-10 py-8 flex-wrap">
		...
		<a href="/toolbox/submit" class="inline-block px-6 py-3 rounded text-white font-sans font-semibold text-sm no-underline flex-none">Submit a resource</a>
	</div>
</div>
